...
class A implements IA {
  ...
  Stack a9 = new Stack();         // Noncompliant {{Replace the synchronized class "Stack" by an unsynchronized one such as "Deque".}}
  ...
  private void f() {
    ...
    a.addAll(new Vector()); // Compliant
    Vector<Integer> result = null; // Noncompliant
    List result2 = new Vector<Integer>(); // Noncompliant
  }
  ...
  public Stack f2() { // Noncompliant {{Replace the synchronized class "Stack" by an unsynchronized one such as "Deque".}}
    return null;
  }
